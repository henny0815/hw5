...
//      T = 1;
  ...
  private void sampleAndSwap(int nodeId) {
    ...
    if (config.getNodeSelectionPolicy() == NodeSelectionPolicy.HYBRID
      ...
      // TODO
    	//get neighbours and pass it to findPartner
    ...
    if (config.getNodeSelectionPolicy() == NodeSelectionPolicy.HYBRID
      ...
      // TODO
    	//get random nodes if partner is still empty
    	if (partner == null){
    ...
    // swap the colors
    // TODO
    //check first whether partner is found
    if (partner != null){
    	//normally: Do handshake -> HOW?
    	//swap colour
    	int c1 = partner.getColor();
    	int c2 = nodep.getColor();
    	partner.setColor(c2);
    	nodep.setColor(c1);
    	this.numberOfSwaps++;
    }
  ...
  public Node findPartner(int nodeId, Integer[] nodes){
    ...
    // TODO
    int colP = nodep.getColor();
    float alpha = this.config.getAlpha();
    for (int i=0; i < nodes.length; i++){
    	// compare the potential partners for exchange that are stored in nodes
    	//get old values
    	int dpp = this.getDegree(nodep, colP);
    	Node nodeq = entireGraph.get(nodes[i]);
    	int colQ = nodeq.getColor();
    	int dqq = this.getDegree(nodeq, colQ);
    	double oldValue = Math.pow(dpp, alpha) + Math.pow(dqq, alpha);
    	
    	//get values if exchanged
    	int dpq = this.getDegree(nodep, colQ);
    	int dqp = this.getDegree(nodeq, colP);
